  ...
  useStitchData(async (root) => {
    ...
    const loadThreadData = async () => {
      try {
        ...
        // Determine if current user is the discussion author
        ...
        const isAuthor = currentUserId !== null && currentUserId === disc.created_by
        ...
        if (replyFeed) {
          ...
          replies.items.forEach((reply) => {
            ...
            replyCard.innerHTML = `
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex gap-4">
                  ...
                  ${isAuthor && !reply.is_accepted ? `<button class="text-secondary-fixed-dim hover:text-secondary-fixed transition-colors font-label-md text-label-md cm-accept-btn flex items-center gap-1"><span class="material-symbols-outlined text-[18px]">check_circle</span> Accept Answer</button>` : ''}
                </div>
              </div>
            `
            ...
            const acceptBtn = replyCard.querySelector('.cm-accept-btn')
            acceptBtn?.addEventListener('click', async () => {
              try {
                await discussionsApi.acceptReply(id, reply.id)
                loadThreadData()
              } catch (err) {
